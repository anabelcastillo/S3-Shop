...
function showFilms(currentFilms) {
  const formatedFilms = currentFilms.map( function (film, i)  {
...
showFilms(films);//------ es la primera cosa que descarga el archivo

//---------------------------------------------------

function searchByCriteria() {
  const searchTitle = document.getElementById("search-value").value;
	
  const filmsfound = films.filter((film) => film.title === searchTitle);

  showFilms(filmsfound);
}
...
function findByGenre(genre) {
  const container = films.filter((film) =>
    film.genre.find((element) => element === genre)
  );
